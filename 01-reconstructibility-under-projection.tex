...
\begin{abstract}
AI systems increasingly reason and act from compressed representations such as prompts, summaries, retrieved passages, logs, metrics, traces, memory, and policy artifacts. Such representations may collapse distinctions required by downstream claims while leaving those claims locally coherent. This paper characterizes that failure as query-relative representational loss rather than ordinary inference error. For a projection \(P:S\rightarrow R\), a query is identifiable only when states collapsed by \(P\) do not require different answers; when that condition fails, additional reasoning over \(R\) cannot universally recover the missing distinction.

The paper develops preserved, reconstructible, traceable, and opaque dependency status as a thin operational interface between projected representation and epistemic operation. The four base cases arise from a gated recovery--attribution structure: direct availability yields preserved status, bounded recovery yields reconstructible status, and attribution distinguishes traceable from opaque loss when recovery is unavailable. Their significance is behavioral. Dependency status constrains licensed operations such as direct inference, bounded reconstruction, retrieval, qualification, routing, or refusal of the requested form. Across a declared family of contexts that distinguishes all four statuses, no proper quotient preserves the same licensing behavior. The framework further separates actual, assessed, and reported dependency status from selected operation, exposing assessment error, reporting inflation, action-governance failure, and unsupported promotion as distinct structural failures.

The objective is not perfect reasoning or complete representation, but diagnostic legibility: preserving enough structure that materially relevant uncertainty, loss, assumptions, and unsupported dependencies can be exposed rather than silently promoted into determinate claims. Representations may be refined as inquiry reveals new dependencies, but such refinement should respond to demonstrated diagnostic need rather than an aspiration to encode all potentially relevant state. The framework therefore treats reliability under projection as disciplined management of visible and correctable failure.
\end{abstract}
...
Dependency status does not by itself select one continuation or compel a system to follow it. It constrains an admissible operation set. Selection among licensed operations may additionally depend on cost, expected diagnostic value, utility, authority, policy, safety, reversibility, and other decision commitments. A proposed action must also satisfy the operational-validity conditions of the declared regime. Accordingly,
...
\begin{enumerate}[leftmargin=1.5em]
    \item It distinguishes correctable inference error from query-relative representational loss and identifies the conditions under which additional reasoning over the same representation cannot recover a missing distinction.
    \item It develops preserved, reconstructible, traceable, and opaque dependency status as a thin operational interface between projected representation and epistemic operation; derives the four base cases through a gated recovery--attribution structure; and distinguishes controller-local compression from the portable status distinctions required by a declared, status-separating family of contexts.
    \item It identifies unsupported promotion as a common structural failure and binds changes in dependency support to changes in licensed operation and response form.
    \item It separates actual, assessed, and reported dependency status from selected operation, while deriving observability, bounded correction, and non-collapse as requirements for keeping those relations diagnostically available during reasoning.
\end{enumerate}

The paper first introduces a minimal case in which projection makes a query non-identifiable. It then develops projection, error, loss, and dependency status; connects status to licensed epistemic operation; and derives the structural requirements for reliable correction. Diagnostic cases show how the same interface applies to scalarization, causal attribution, provenance, decision under non-identifiability, and agentic action. The final sections relate the framework to neighboring methods, describe its operational and cybernetic implications, and clarify its implementation, evaluation, and representational limits.
...
This example contains the paper's complete operational problem in minimal form. Projection determines what distinctions survive; dependency status characterizes the support that remains; admissibility determines which epistemic operations are available; and separate decision or policy considerations select among the licensed continuations.
...
Treating all three as merely ``represented'' would preserve their presence while collapsing the epistemic role each one actually plays. This is an example of role non-collapse rather than a claim that the operative representation contains three independent agents or subsystems: the same participant or implementation may carry several such roles, provided their different licensing effects remain reconstructible.

More generally, provenance of support need not identify the structure that actually selected an outcome among alternatives. Determination provenance makes this distinction explicit by asking which represented commitments or choices were outcome-determining rather than merely present in the derivation history~\cite{Hellerstein2026Determination}. From the present perspective, determination structure is itself a query-relative dependency that may survive one projection while being erased by another.

The same typed-support principle applies to influence roles themselves. Preserving a source and its factual content does not necessarily preserve whether that source was permitted merely to support, challenge, trigger, or select a downstream continuation. Such governance structure is another query-relative dependency that may be preserved, reconstructible, or lost under projection.
...
The richer state \(S\) need not be an inaccessible world state. It may itself be an explicit persistent reasoning representation containing more history, dependencies, provenance, alternatives, and lifecycle information than can or should be loaded for one reasoning step. In that case, query-relative retrieval from the persistent state into a bounded operative context is itself a projection:

\[
P_q:B\rightarrow R_q,
\]

where \(B\) is a richer persistent reasoning state and \(R_q\) is the bounded representation assembled for query \(q\).

This architecture does not eliminate representational loss. It changes where projection occurs and can make that projection more explicit, repeatable, and reversible while the richer source state remains legitimately available for later retrieval. If that state later expires, is minimized, or is deleted under an applicable governance regime, subsequent reconstructibility may correspondingly weaken or disappear.

\[
\boxed{
\text{persistent state}
\rightarrow
\text{bounded query-relative projection}
\neq
\text{projection-free reasoning}.
}
\]

Persistence in this architectural sense is not permanence. The framework requires support claims to track the information basis that is actually available for the current use; it does not require every richer representation to remain available indefinitely.

The richer persistent source state need not be realized as one monolithic artifact. It may instead be distributed across versioned reasoning objects in which domain reasoning, governance state, policy state, protected semantics, and other independently authoritative components remain separately represented while contributing selectively to a bounded operative projection. This architectural distribution does not alter the projection problem: whatever subset of that distributed state is retrieved, exposed, or otherwise made available for the current query constitutes the operative information basis against which support and admissibility must be assessed.

\[
\boxed{
\text{distributed persistent state}
\neq
\text{current operative support}.
}
\]
...
Equivalently, projection determines which distinctions remain observable through the representation. In measure-theoretic terms, this observable distinction structure can be expressed through a representation-induced \(\sigma\)-algebra: a query is directly supported only when the distinction it requires is measurable under the projected representation, or becomes available through bounded reconstruction.
...
Thus the term \emph{ladder} refers to an operational weakening of available response forms, not to a claim that all support relations form a total epistemic order or a single scalar dimension. The corresponding minimality claim is conditional and behavioral rather than metaphysical. A particular controller may admit a coarser partition when its current operation repertoire treats two statuses identically. Across a declared family of contexts that operationally distinguishes all four statuses, however, no proper quotient preserves the same licensing behavior. More fine-grained statuses such as probabilistic reconstruction, partial reconstruction, conflicting provenance, or prior-dependent inference may refine the four base cases when the governing operation family distinguishes those refinements.

The operational significance of the partition is therefore stronger than a four-way confidence label. Let \(\mathcal{O}_{\Gamma,q}(\delta)\) denote the set of epistemic operations licensed for dependency status \(\delta\) under regime \(\Gamma\) and query \(q\). This is the same operation-set notation introduced in the definition of structural reliability above. The statuses matter because, in general,

\[
\mathcal{O}_{\Gamma,q}(\text{preserved})
\neq
\mathcal{O}_{\Gamma,q}(\text{reconstructible})
\neq
\mathcal{O}_{\Gamma,q}(\text{traceable})
\neq
\mathcal{O}_{\Gamma,q}(\text{opaque}).
\]

For example, reconstruction may be required rather than abstention when a dependency is boundedly reconstructible, while retrieval or routing may be appropriate when it is traceable but not reconstructible. Structural status therefore constrains the operation space rather than simply supplying a scalar degree of epistemic confidence.
...
\paragraph{A gated rather than unrestricted product.}
The four statuses do not arise from treating recoverability and auditability as an unrestricted product. Such a product would produce six combinations. In the present semantics, attribution becomes status-resolving specifically when recovery is unavailable. Direct availability yields preserved status, bounded recovery yields reconstructible status, and attribution distinguishes the two cases remaining after recovery fails. The resulting conditional structure is
...
This construction explains why traceable is not simply an intermediate quantity of reconstruction. It records a different residual capability: the distinction cannot presently be recovered, but the missing dependency can still be localized, inspected, retrieved, routed, contested, or governed in ways unavailable when the dependency is opaque.

\paragraph{Local behavioral equivalence.}
The number of statuses a particular controller must retain depends on the operations available to that controller. Let

\[
O_{\mathrm{ep}}
\]

denote the declared epistemic-operation set, let

\[
\eta=(\Gamma,q)
\]

denote a query--regime context, and let

\[
\Lambda_{\eta}:\Sigma\times O_{\mathrm{ep}}\rightarrow L
\]

be the licensing relation, where \(L\) contains the licensing outcomes relevant to the regime. Once this relation has been specified independently of a proposed coarsening of \(\Sigma\), define

\[
s\equiv_{\eta}t
\quad\Longleftrightarrow\quad
\forall o\in O_{\mathrm{ep}},
\;
\Lambda_{\eta}(s,o)=\Lambda_{\eta}(t,o).
\]

Statuses \(s\) and \(t\) are locally behaviorally equivalent when the current context licenses exactly the same operations for both. A controller may compile locally equivalent statuses into one internal control state without changing its present licensing behavior. For example, a controller with no retrieval, routing, provenance-following, or escalation operation may treat traceable and opaque loss identically.

\paragraph{Portable behavioral equivalence.}
Local equivalence does not establish that the same compression is safe when represented state crosses controller or regime boundaries. For a declared family of contexts \(\mathfrak C\), define
...
is the coarsest status abstraction that preserves the declared licensing behavior across \(\mathfrak C\). It retains distinctions because they are consequential to some intended continuation, not because retaining more information is intrinsically preferable.
...
\exists o\in O_{\mathrm{ep}}
\quad
\Lambda_{\eta}(s,o)\neq\Lambda_{\eta}(t,o).
\]

For a status-separating family,
...
Consequently, no proper quotient of the four-status interface preserves epistemic-operation licensing across that family. If a proposed quotient identifies two distinct statuses, status separation supplies a context and operation for which their licensing outcomes differ. Any licensing rule factored through the proposed quotient would have to assign the same outcome to both, contradicting that witness. Conversely, quotienting by \(\equiv_{\mathfrak C}\) is licensing-preserving by construction. The coarsest licensing-preserving quotient is therefore unique up to isomorphism for the declared context family and licensing relation.

This is the precise sense in which the four-status interface may be minimal. It does not claim that every controller requires four internal states or that every possible reasoning regime produces exactly four epistemic categories. It says that when the declared context family distinguishes preserved, reconstructible, traceable, and opaque through their licensed continuations, no smaller interface preserves that behavior. Thus,
...
The portable admissibility quotient justifies dependency status as a control interface: a local implementation may minimize its controller state, while a portable representation should preserve every status distinction that remains behaviorally consequential across its declared recipient family. The result does not require universal retention of all represented information. It requires retention of the distinctions needed to preserve the declared licensing behavior across the boundary for which portability is claimed.
...
\[
\Lambda_{\Gamma,q}(\delta,o) \in \{\text{licensed},\text{conditional},\text{unlicensed}\},
\]
...
\[
\mathcal{O}_{\Gamma,q}(\delta) = \left\{o : \Lambda_{\Gamma,q}(\delta,o)=\text{licensed}\right\}.
\]

At the claim level, \(\mathcal{O}_{\Gamma,q}(\Delta)\) denotes the operation set licensed by the relevant dependency-status profile rather than by any one dependency considered in isolation.

This representation makes explicit why dependency status is not merely a confidence scale. A change in status can change the \emph{kind} of epistemic operation available to the reasoner rather than merely changing the confidence with which one fixed operation is performed.
...
A later projection that preserves a claim while omitting a newly discovered material constraint may therefore license an answer form stronger than the revised reasoning state supports. Call this failure \emph{silent constraint erasure}: a constraint discovered during inquiry ceases to govern a dependent claim because a later representation drops the constraint while retaining the object it limited.
...
\paragraph{Unsupported promotion.}
The three structural requirements can be understood as controls against a common failure mechanism: \emph{unsupported promotion}. Call a transition an unsupported promotion when a reasoning process treats a weaker, differently typed, or merely retained support relation as though it established the additional conditions required for a stronger epistemic license, without an explicit represented witness for that strengthening. Observability helps expose candidate promotions, bounded correction prevents hidden strengthening during repair, and non-collapse prevents support from being transferred illegitimately across representational roles or levels.
...
\subsection{Licensing, Selection, and the Decision to Continue}

Dependency status constrains the operation set; it does not generally rank every operation that remains within that set. Let

\[
\mathcal O_{\Gamma,q}
\bigl(
\widehat{\Delta}_{\Gamma,q,R}
\bigr)
\]

denote the epistemic operations licensed by the assessed dependency-status profile. Selection among those operations may depend on additional represented considerations such as interaction or computational budget \(B\), expected diagnostic value, reversibility, urgency, utility, authority, and a declared selection policy \(\Pi\). Schematically,
...
\widehat{\Delta}_{\Gamma,q,R}
\bigr)
\xrightarrow[\;B,\Pi\;]{\text{selection}}
o_{\mathrm{selected}}.
\]

These additional considerations may choose among operations that are already licensed, but they do not enlarge the licensed operation set by supplying missing epistemic support. A low-cost continuation does not become supported merely because it is inexpensive, and a high-utility action does not become evidence-determined merely because policy favors it.
...
\[
o_{\mathrm{selected}} \in \mathcal{O}_{\Gamma,q}\bigl(\widehat{\Delta}_{\Gamma,q,R}\bigr)
\]

denote the epistemic or operational continuation selected by the system from the operation set licensed by its assessed dependency-status profile and governing regime. This separation exposes a third failure mode. An \emph{action-governance failure} occurs when the system's selected continuation is not licensed even by its own assessment:

\[
o_{\mathrm{selected}} \notin \mathcal{O}_{\Gamma,q}\bigl(\widehat{\Delta}_{\Gamma,q,R}\bigr).
\]

A different case occurs when the system faithfully follows the operation policy induced by \(\widehat{\Delta}_{\Gamma,q,R}\), but \(\widehat{\Delta}_{\Gamma,q,R}\neq\Delta^{*}_{\Gamma,q,R}\). The system may then be governance-compliant relative to a mistaken structural assessment while still selecting an operation that the actual dependency-status profile does not license. Assessment correctness and governance compliance are therefore separate properties.
...
The purpose of this section is not to define a complete runtime protocol, evaluation framework, or adaptive control implementation. Its narrower purpose is to show how the dependency-status interface yields operational targets: answer-form governance, evaluation under projection stress, interface-level representation repair, and audit fields for downstream systems.
...
These requirements are conceptual design constraints in the present paper. The paper does not claim to establish their universal necessity, independence, or sufficiency across every reasoning regime. They should not be interpreted as a complete theory of reasoning or as substitutes for calibration, provenance, abstraction refinement, uncertainty representation, or compositional analysis. Their narrower role is to preserve the structural distinctions required for dependency status to continue governing epistemic operation.
...
This separates \emph{determination provenance} from \emph{determination permission}. A representation may correctly preserve which source or commitment selected an outcome while still losing the source--target--role constraint needed to establish that the represented determination was licensed.
...
Evidence, background assumptions, and policy constraints are instead distinct support roles within one operative information basis. Target formation and representational projection are distinct transformations in a composed process. Elsewhere in the framework, two reasoners or two reasoning bundles may be used as the smallest illustrative case even though the relevant structure extends to
...
The term \emph{reconstructability} also has an established systems-theoretic use in Reconstructability Analysis (RA), developed from Ashby's constraint analysis and subsequently systematized by Klir and others~\cite{KlirWay1985,Klir1986RA}. RA studies relations between systems perceived as wholes and their subsystems, including identification of possible whole-system structures from subsystem relations and determination of subsystem structures from which a whole can be reconstructed to an acceptable approximation. The present framework addresses a different reconstruction problem. Given a projection used by a bounded reasoner, it asks whether the distinctions required by a particular downstream claim remain preserved or boundedly reconstructible and, when they do not, how the licensed response form must change. Thus RA primarily concerns reconstruction of system structure from relational decomposition, whereas reconstructibility under projection concerns query-relative claim licensing under representational loss.
...
Database theory provides another close comparison through the theory of certain answers under incomplete information. Certain answers are ordinarily characterized by what remains true across the complete databases represented by an incomplete database~\cite{Libkin2016}. This shares the present framework's refusal to treat unresolved alternatives as supporting an arbitrary determinate answer. The present account asks a further operational question: when the requested answer is not invariant across represented possibilities, does the operative representation still permit bounded reconstruction, preserve an attributable dependency that can be retrieved or routed, or leave the dependency opaque? Certain-answer semantics identifies what can be asserted across completions; dependency status additionally governs which recovery, qualification, tracing, routing, decision, or refusal operation remains licensed when such assertion is unavailable.

Another close neighboring literature concerns awareness and unawareness in epistemic logic. Fagin and Halpern distinguish ordinary limitations of belief from cases in which an agent must first be aware of a concept before beliefs concerning it are available to the agent~\cite{FaginHalpern1987}. This is closely related to, but not identical with, the distinction between traceable and opaque dependency loss developed here. A traceable dependency preserves enough meta-structure for the reasoner to represent that a potentially material dependency exists even though the underlying distinction cannot presently be recovered. In the opaque case, no adequate attributable dependency trail remains within the operative representation. Thus the difference between ``the relevant distinction is unavailable'' and ``the representation does not make the missing dependency available as something to ask about'' is structurally important.

The present framework adds a query-relative operational consequence to this distinction. Awareness and unawareness logics primarily characterize limits on what propositions or concepts are available to an agent's epistemic state. Reconstructibility under projection asks additionally what response form is licensed when a query depends on such unavailable structure. Traceability may still license marking, qualification, routing, or representation expansion; opacity cannot silently be promoted into determinate support merely because the reasoner fails to represent what was lost.
...
An especially relevant empirical result comes from Slobodkin et al., who find that large language models can encode information predictive of whether a question is answerable while nevertheless generating a hallucinatory answer~\cite{Slobodkin2023}. This illustrates a failure that is not naturally described as absence of all query-relevant representation: a distinction relevant to answerability may be represented or diagnostically recoverable without successfully governing the emitted response. In the terminology developed here, representational support, dependency assessment, and report or action selection are therefore separable failure locations. The result does not establish the present framework, but it provides empirical motivation for treating coupling between structural assessment and licensed output as a reliability problem in its own right.

Across these traditions, the closest point of contact is the treatment of projection failure as a failure of query-relative sufficiency. The contribution of the present framework is not another general proof that compression can destroy task-relevant information. It introduces dependency status as an intermediate control interface between representational support and permissible continuation. The interface records whether a required dependency is directly preserved, boundedly reconstructible, merely attributable, or opaque; binds those conditions to different epistemic operations and response forms; and separates the status that actually obtains from the status a bounded reasoner assesses and reports. Its unification is therefore operational rather than ontological: heterogeneous domains retain their own objects and methods while sharing a small interface for governing what may follow from a projected representation. The local and portable behavioral quotients further specify when that interface may be compressed for one controller and when its distinctions must survive transfer across a declared family of contexts.
...
The framework has a cybernetic interpretation, but dependency status should not be confused with a complete controller state. A bounded reasoner receives or constructs an operative representation, assesses the dependencies required by a proposed continuation, restricts the available operation set, selects among licensed continuations under separate budget and policy considerations, and observes the resulting consequences. Those observations may reveal ordinary error, expose a previously unresolved dependency, or create diagnostic pressure to refine the representation.
...
where \(R_t\) is the operative representation, \(\widehat{\Delta}_t\) is the assessed dependency-status profile, \(\mathcal O_t\) is the licensed operation set, \(o_t\) is the selected continuation, and \(y_t\) is the observed consequence or feedback available for later reasoning. The transition from \(y_t\) to \(R_{t+1}\) may leave the representation unchanged, correct an error within it, retrieve or reconstruct missing support, or refine the represented dependency structure.
...
Question formation can also reveal dependencies that were absent from the initial problem model. A clarification may split one overloaded query into several queries, expose a missing stakeholder, distinguish evidence from authority, or make an existing concern newly relevant to a downstream action. Such events do more than enrich the current representation with additional facts. They revise the structure to which admissibility is applied. The seventh paper develops this process as constraint discovery and contextual persistence: newly discovered relationships should remain attached to the claims, actions, questions, or projections they limit until their disposition is made explicit.
...
These limitations are not defects of the account. They follow from projection and from the developmental incompleteness of practical reasoning. A system may faithfully govern the dependency structure it currently represents while still failing to notice a material relationship. Newly discovered constraints may revise that structure and should continue to govern dependent claims while they remain materially operative and legitimately retained, but discovery can never be assumed complete and represented state need not be retained permanently. A system may likewise preserve and correctly execute a reasoning procedure while failing to represent a condition that limits when that procedure is applicable.
...
\subsection{Implementation and Evaluation Scope}
...
The present paper therefore specifies a conceptual architecture, a basic non-identifiability result, a dependency-status interface, and the operational consequences of that interface. It does not provide a complete detector, runtime enforcement protocol, adaptive controller, or empirical validation of the proposed distinctions. Those remain implementation and evaluation problems rather than conclusions that follow from the formal definitions alone.
...
AI systems ordinarily reason and act under projection. Prompts, summaries, retrieved passages, logs, metrics, memories, tool outputs, and policy artifacts are bounded representations of richer states. Such representations may support fluent and locally valid reasoning while collapsing a distinction required by a downstream claim or action. When states that require different answers are indistinguishable under the operative projection, additional reasoning over that representation cannot universally recover the missing distinction.

The paper's contribution is operational rather than a new discovery that projection can destroy information. It introduces dependency status as a thin waist between heterogeneous representations and heterogeneous reasoning workflows:
...
Preserved, reconstructible, traceable, and opaque status arise from a gated recovery--attribution structure. Directly available support licenses different continuations from support that must first be reconstructed; an attributable pointer to unavailable support licenses different continuations from opacity. These statuses are not a universal ontology or a four-level confidence scale. They are an operational partition whose required granularity depends on the licensing behavior that an interface must preserve. A controller may safely merge statuses it treats identically, but local irrelevance does not establish portable irrelevance. Across a status-separating family of contexts, no proper quotient of the four-status interface preserves the same licensing behavior.

The recurring failure is unsupported promotion: a downstream object, claim, procedure, or action acquires a stronger epistemic role than its represented support warrants. Traceability does not silently become reconstruction; policy selection does not become evidential determination; successful execution does not certify applicability; and a claim preserved during transport does not thereby become adequate for its destination. Preventing such promotion requires structural support, operational permission, and continuation selection to remain distinguishable.

Practical governance also requires separating the dependency status that actually obtains from the status a bounded reasoner assesses, the status it reports, and the operation it selects. A system may comply with its operation policy while acting on a mistaken assessment, report its assessment faithfully while that assessment is wrong, or assess its support correctly while selecting an unlicensed continuation. Factual correctness, assessment fidelity, reporting fidelity, and governance compliance are therefore distinct evaluation targets.

The framework has a cybernetic consequence. Dependency status can serve as a compact control variable through which a bounded reasoner changes its operation when its information basis changes. Feedback may motivate correction, retrieval, qualification, routing, or representation refinement, but this is a corrigibility mechanism rather than a convergence or completeness theorem. A governance procedure remains subject to projection and cannot infer from its own successful execution that every dependency required to certify its application was represented.

Structural reliability under projection should therefore not be understood as approximation to an omniscient reasoner. Its more attainable objective is diagnostic legibility: preserving enough information about dependencies, recovery paths, attribution, assumptions, and loss that unsupported continuation becomes visible and that demonstrated failures can exert pressure on later representation and control. The aim is not to prevent every error, but to keep a bounded system from silently treating what it can trace, assume, select, or execute as though it had thereby reconstructed what its representation does not support.
