...
The distinction therefore introduces two members of the broader failure taxonomy developed below. \emph{Assessment failure} occurs when the reasoner misclassifies reference dependency status. \emph{Reporting failure} occurs when the answer communicates represented-support commitments stronger than the reasoner's own assessment licenses. Runtime governance can directly constrain reporting failure; assessment failure remains subject to the same projection limits that motivate the framework as a whole.
