...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item provenance that a source participated in producing a result with permission for that source to determine the result in the role it played,
...
These boundaries correspond to an intentional division of labor across the stack. Paper~2 supplies the formal necessity, independence, and bounded sufficiency results for the reconstructibility conditions. Paper~4 turns dependency status and operational validity into explicit runtime response-form governance. Paper~5 operationalizes projection-sensitive behavior through a structural evaluation harness, including matched changes in representation and response form. The present paper therefore need not duplicate those formal, protocol, or empirical layers in order to state the conceptual licensing problem they develop.
