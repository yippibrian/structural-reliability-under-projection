...
The paper develops a dependency-status account---preserved, reconstructible, traceable, and opaque---and binds those structural support conditions to the epistemic operations they license: direct inference, bounded reconstruction, trace or retrieval, qualification, routing, or refusal of the requested form. It separates informational support from operational permission and distinguishes actual, assessed, and reported dependency status from the action or continuation ultimately selected. The resulting framework treats structural insufficiency as a constraint on available epistemic operations rather than merely a reason to lower confidence, while retaining a second-order limit on self-certifying admissibility procedures.
...
The operational significance of the partition is therefore stronger than a four-way confidence label. Let \(A_{\Gamma}(d)\) denote the set of epistemic operations licensed for dependency status \(d\) under regime \(\Gamma\). The statuses matter because, in general,

\[
A_{\Gamma}(\text{preserved}) \neq A_{\Gamma}(\text{reconstructible}) \neq A_{\Gamma}(\text{traceable}) \neq A_{\Gamma}(\text{opaque}).
\]
