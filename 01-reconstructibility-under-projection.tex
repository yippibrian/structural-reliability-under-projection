...
\neq
\text{policy prescription}
\neq
\text{actual selection}.
}
\]
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item It derives preserved, reconstructible, traceable, and opaque status from a gated recovery--attribution construction, expresses that construction through three nested capability thresholds, and proves under stated sufficiency, separation, and surjectivity assumptions when the statuses are exactly the primitive behavioral classes.
    \item It characterizes dependency status as a thin licensing interface, distinguishes controller-local compression from portable status non-collapse, and separates epistemic licensing, operational validity, admissibility, policy prescription, and actual selection.
    \item It identifies unsupported promotion as a common structural failure and separates actual, assessed, and reported dependency status from the policy-prescribed operation and the operation actually selected.
...
Confidence is a further distinct variable. It describes a reasoner's degree of belief or uncertainty about an answer; it does not establish whether the operative representation preserves the distinction required to determine that answer. An identifiable query may receive low confidence because the reasoner is unreliable or uncertain, while a non-identifiable query may receive high confidence because the reasoner fails to recognize the projection boundary. Recent work on LLM assessment provides a concrete example of this separation: \emph{Lost in Projection} reports that assessment-state representations can be decoded and manipulated without thereby changing task performance, so an internal assessment signal should not be treated as a certificate of competence~\cite{LostProjection2026}. Therefore,
...
\begin{definition}[Nested capability thresholds]
\label{def:nested-capability-thresholds}
For the primitive support space \(X_{\eta}\), define

\[
E_{1,\eta}=\{x\in X_{\eta}:\rho_{\eta}(x)=\mathsf{direct}\},
\]

\[
E_{2,\eta}=\{x\in X_{\eta}:\rho_{\eta}(x)\in\{\mathsf{direct},\mathsf{bounded}\}\},
\]

and

\[
E_{3,\eta}=E_{2,\eta}\cup\{x\in X_{\eta}:\rho_{\eta}(x)=\mathsf{unavailable}\ \text{and}\ \alpha_{\eta}(x)=\mathsf{attributable}\}.
\]

These sets satisfy \(E_{1,\eta}\subseteq E_{2,\eta}\subseteq E_{3,\eta}\subseteq X_{\eta}\) by construction. Their four regions are

\[
E_{1,\eta},\qquad E_{2,\eta}\setminus E_{1,\eta},\qquad E_{3,\eta}\setminus E_{2,\eta},\qquad X_{\eta}\setminus E_{3,\eta},
\]

corresponding respectively to \(\mathsf P,\mathsf R,\mathsf T,\mathsf O\). Define the associated capability signature by

\[
\chi_{\eta}(x)=\bigl(\mathbf{1}_{E_{1,\eta}}(x),\mathbf{1}_{E_{2,\eta}}(x),\mathbf{1}_{E_{3,\eta}}(x)\bigr).
\]

Its possible values are

\[
(1,1,1),\qquad(0,1,1),\qquad(0,0,1),\qquad(0,0,0),
\]

and \(\kappa_{\eta}\) is obtained by naming these signatures \(\mathsf P,\mathsf R,\mathsf T,\mathsf O\), respectively.
\end{definition}

The nested construction does not introduce primitives independent of \(\rho_{\eta}\) and \(\alpha_{\eta}\). It reorganizes the same gated recovery--attribution conditions as three separately testable capability thresholds. The inclusions are automatic; surjectivity of \(\kappa_{\eta}\) is what ensures that all four regions are nonempty and hence that all four signatures are realized.
...
Because \(\chi_{\eta}\) and \(\kappa_{\eta}\) have exactly the same fibers, status sufficiency may equivalently be stated as capability sufficiency:

\[
\chi_{\eta}(x)=\chi_{\eta}(y)\quad\Longrightarrow\quad\forall o\in O_{\mathrm{base}},\;\lambda_{\eta}(x,o)=\lambda_{\eta}(y,o).
\]
...
\begin{definition}[Capability-threshold witness]
\label{def:capability-threshold-witness}
For \(k\in\{1,2,3\}\), an operation \(o_k\in O_{\mathrm{base}}\) is a witness for \(E_{k,\eta}\) when

\[
x\in E_{k,\eta},\ y\notin E_{k,\eta}\quad\Longrightarrow\quad\lambda_{\eta}(x,o_k)\neq\lambda_{\eta}(y,o_k).
\]
...
\begin{proposition}[Threshold witnesses imply fiber separation]
\label{prop:threshold-witness-separation}
If \(E_{1,\eta}\), \(E_{2,\eta}\), and \(E_{3,\eta}\) admit a threshold-witnessing family, then \(\lambda_{\eta}\) is fiber-separating.
\end{proposition}

\begin{proof}
If \(\kappa_{\eta}(x)\neq\kappa_{\eta}(y)\), then the corresponding nested capability signatures satisfy \(\chi_{\eta}(x)\neq\chi_{\eta}(y)\). Hence their membership differs for at least one threshold \(E_{k,\eta}\). The witness \(o_k\) for that threshold satisfies \(\lambda_{\eta}(x,o_k)\neq\lambda_{\eta}(y,o_k)\), so the two primitive support conditions are behaviorally distinguishable.
\end{proof}
...
\begin{corollary}[Witnessed primitive behavioral completeness]
\label{cor:witnessed-primitive-behavioral-completeness}
Suppose that \(\kappa_{\eta}\) is surjective, \(\lambda_{\eta}\) is status-sufficient, and the three nested capability thresholds admit a threshold-witnessing family. Then
\[
...
\begin{proof}
Proposition~\ref{prop:threshold-witness-separation} supplies fiber separation, after which Theorem~\ref{thm:primitive-behavioral-completeness} applies.
\end{proof}

Theorem~\ref{thm:primitive-behavioral-completeness} gives the general primitive-to-status result, while Corollary~\ref{cor:witnessed-primitive-behavioral-completeness} supplies a concrete sufficient route through three threshold-witnessing operations. The nested construction makes the separation hypothesis inspectable: different status regions cross different recovery or attribution thresholds, and a declared operation family preserves all four behavioral classes when it contains operations that witness those crossings. This is a reorganization and partial strengthening of the theorem's hypotheses, not a derivation from primitives independent of the gated recovery--attribution construction. The result remains conditional on the recovery and attribution semantics, operation family, and assessment context; it is not a claim that reality contains exactly four epistemic states.
...
\begin{enumerate}[leftmargin=1.5em]
    \item \emph{Primitive behavioral completeness} asks whether primitive support conditions factor through exactly the four base status classes for the declared operation family. Theorem~\ref{thm:primitive-behavioral-completeness} answers this under surjectivity, status-sufficiency, and fiber-separation assumptions, while Corollary~\ref{cor:witnessed-primitive-behavioral-completeness} shows that three capability-threshold witnesses are sufficient to establish the required separation.
...
Combination may also change the destination evidence state itself. Two representations that independently leave a dependency unreconstructible may jointly expose enough relational structure to reconstruct it, even though neither source assessment is strengthened or rewritten. Thus conservative preservation of source-relative status does not imply invariance of destination-relative inferential capacity; composition may require reassessment because it can reveal distinctions unavailable in either input alone.

\[
\boxed{
\text{source-status preservation}
\not\Rightarrow
\text{destination inferential invariance}.
}
\]
...
\(^{\dagger}\)Direct inference is not licensed while the dependency remains reconstructible. Successful bounded recovery changes the operative information state so that the recovered distinction is preserved, after which direct inference may become licensed.
...
In this section, an \emph{epistemic operation} is a reasoning or communicative move such as inference, reconstruction, qualification, or retrieval. An \emph{operational continuation} is the larger system move that may perform, communicate, or act on an epistemic result. The notation \(\mathcal O_{\eta}\) ranges over candidate continuations: \(\mathcal O^{\mathrm{ep}}_{\eta}\) retains those whose epistemic component is licensed by dependency status, while \(\mathcal O^{\mathrm{valid}}_{\eta}\) separately retains those permitted by the governing regime. A purely epistemic continuation may have a trivial operational-validity condition; an action-bearing continuation need not.
...
\subsection{Actual, Assessed, Reported, Prescribed, and Selected State}
\label{subsec:status-assessment-selection}

Runtime governance requires a distinction between actual, assessed, and reported dependency status, together with the operation prescribed by a compliant policy and the operation actually selected by the system. For each dependency \(d\) under evaluation, let \(x^{*}_{\eta}(d)\in X_{\eta}\) denote the primitive support condition that semantically obtains in assessment context \(\eta=(\Gamma,q,R)\) under the declared recovery and attribution semantics. Define the actual, or reference, dependency status by \(\delta^{*}_{\Gamma,q,R}(d)=\kappa_{\eta}\bigl(x^{*}_{\eta}(d)\bigr)\). This is a semantic property of the relation among the dependency, operative representation, recovery conditions, query, and regime; it need not be observable, computable, or correctly assessable by the bounded reasoner operating on \(R\). Let \(\widehat{\delta}_{\Gamma,q,R}(d)\) denote the status assessed by the reasoner, and let \(\widetilde{\delta}_{\Gamma,q,R}(d)\) denote the status reported in the resulting answer or reasoning artifact. Collectively, these induce actual, assessed, and reported dependency-status profiles \(\Delta^{*}_{\Gamma,q,R}\), \(\widehat{\Delta}_{\Gamma,q,R}\), and \(\widetilde{\Delta}_{\Gamma,q,R}\) over the dependencies currently treated as material to the query. The normative framework constrains response form relative to the reference profile, while practical systems can act only through imperfect assessed profiles. Evaluation must therefore consider both whether the assessed profile is accurate and whether the reported answer form faithfully reflects that assessment.
...
\[
o_{\mathrm{selected}}\in\mathcal O_{\eta}
\]
...
An \emph{action-governance failure} occurs when this compliance condition fails:
...
\begin{itemize}[leftmargin=1.5em]
    \item revisions do not silently change the information basis of the representation;
    \item the distinction between correction within \(R\) and refinement to \(R'\) remains explicit; and
    \item external retrieval, intervention, or information acquisition is marked as a change of information basis rather than presented as support already contained in the original representation.
\end{itemize}
...
&
When should a predictor abstain, return a prediction set, or flag unsupported generation?~\cite{GeifmanElYaniv2017,GeifmanElYaniv2019,Vovk2005,Shafer2008,Ji2023,Manakul2023,Hu2026HARP}
&
...
The separation between representation, assessment, and output also has empirical motivation. Slobodkin et al.\ find that language models may encode information predictive of question answerability while still producing hallucinatory answers~\cite{Slobodkin2023}. \emph{HARP} similarly reports that model-specific projections of hidden states onto a residual subspace can improve hallucination detection even when that subspace contributes little to ordinary token prediction~\cite{Hu2026HARP}. These results do not establish the present theory or justify treating the recovered subspace as a universal semantic category; they instead illustrate how an ordinary output projection may suppress or fail to expose information useful for assessment. Together with the assessment--competence dissociation discussed above, they motivate evaluating represented support, assessed support, reported support, and selected behavior separately.
...
\begin{enumerate}
    ...
    \item For a proposed action or continuation, check whether it satisfies the operational-validity conditions of the declared regime or preserves an explicitly permitted path for establishing them.
...
The distinctions among actual, assessed, and reported dependency status and between prescribed and actually selected operations, together with their corresponding failure taxonomy, are developed in Section~\ref{subsec:status-assessment-selection}. The remaining subsections show how those distinctions support evaluation, reasoning scaffolds, interface design, question formation, and audit.
...
\longrightarrow
o_{\mathrm{prescribed},t}
\longrightarrow
o_{\mathrm{selected},t}
\longrightarrow
y_t
\longrightarrow
R_{t+1}.
...
Evaluation should therefore test not only whether a system reaches a target answer, but whether it changes behavior appropriately as the representation changes. One natural benchmark pattern is to hold the semantic query, task wording, and underlying answer fixed while varying only the availability of a material dependency: preserved cases should license direct inference; reconstructible cases should provide a bounded recovery path; traceable cases should preserve attribution without providing source-level recovery; and opaque cases should preserve neither recovery nor an actionable dependency trail. The status should arise from the information structure rather than being named in the prompt. The primary endpoint should be operation selection rather than confidence: infer, reconstruct, retrieve or trace, qualify, defer, route, or refuse as appropriate to the declared status and regime.

A stronger version should begin with an oracle-status condition in which the correct dependency status is supplied to the controller, isolating whether status-conditioned operation selection works. A second condition should require the system to assess the status from the representation, allowing assessment failure to be distinguished from operation-selection failure. The benchmark should report unsupported-promotion rates, especially the treatment of traceable or opaque dependencies as reconstructed, alongside answer accuracy, recovery success, reporting fidelity, and operational-validity compliance.
...
Evidence against the practical importance of the framework would include failure to observe meaningful behavioral differences between preserved, reconstructible, traceable, and opaque dependency conditions under independently designed evaluation; failure of controlled representation enrichment to produce the predicted change in licensed answer form; failure of relational or dependency structure to add diagnostic value beyond component-level information; or repeated external cases in which the framework's distinctions provide no useful explanatory or corrective advantage over simpler existing accounts. Such findings would not invalidate the elementary non-identifiability result itself, but they would weaken the claim that reconstructibility under projection identifies a practically important structure of AI reasoning failure. A particularly direct negative result would be failure to distinguish reconstructible recovery from trace-only provenance or trace-only provenance from opacity at the level of operation selection, even when the semantic task and answer remain fixed.
...
\bibitem{Hu2026HARP}
Junjie Hu, Gang Tu, ShengYu Cheng, Jinxin Li, Jinting Wang, Rui Chen, Zhilong Zhou, and Dongbo Shan.
\newblock HARP: Hallucination Detection via Reasoning Subspace Projection.
\newblock In \emph{Proceedings of the Fourteenth International Conference on Learning Representations}, 2026.
\newblock arXiv:2509.11536.
...
\bibitem{LostProjection2026}
Anonymous authors.
\newblock Lost in Projection: The Geometric Orthogonality of LLM Assessment and Reasoning.
\newblock ACL ARR 2026 May submission, OpenReview, 2026.
