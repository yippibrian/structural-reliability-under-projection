...
Let \(\delta^*_{\Gamma,q,R}\) denote the dependency status that actually obtains for the distinctions required by query \(q\) under representation \(R\) and regime \(\Gamma\), and let \(\widehat{\delta}_{\Gamma,q,R}\) denote the status assessed by a bounded reasoner. Let \(\mathcal{O}_{\Gamma,q}(\delta)\) denote the set of epistemic operations licensed by dependency status \(\delta\).
...
\in
\mathcal{O}_{\Gamma,q}
\bigl(\delta^*_{\Gamma,q,R}\bigr).
\]

Structural reliability should therefore be distinguished from two related runtime properties. \emph{Assessment fidelity} concerns whether
...
L_{\Gamma,q}(\delta,o)
\in
\{\text{licensed},\text{conditional},\text{unlicensed}\},
\]
where \(\delta\) is an assessed dependency status and \(o\) is an epistemic operation. The fully licensed operation set is then
\[
\mathcal{O}_{\Gamma,q}(\delta)
=
\left\{
o :
L_{\Gamma,q}(\delta,o)=\text{licensed}
\right\}.
\]
...
The failure modes above share a more general structure. Call a transition an \emph{unsupported promotion} when a reasoning process treats a weaker, different, or merely retained support relation as though it established the additional conditions required for a stronger epistemic license, without an explicit represented witness for that strengthening.
...
\[
a_{\mathrm{selected}} \in A_{\Gamma}(\widehat{\delta})
\]
denote the epistemic or operational continuation selected by the system from the set licensed by its assessed dependency status and governing regime. This separation exposes a third failure mode. An \emph{action-governance failure} occurs when the system's selected continuation is not licensed even by its own assessment:
\[
a_{\mathrm{selected}} \notin A_{\Gamma}(\widehat{\delta}).
\]
A different case occurs when the system faithfully follows the policy induced by \(\widehat{\delta}\), but \(\widehat{\delta}\neq\delta^{*}\). The system may then be policy-compliant relative to a mistaken structural assessment while still selecting an operation that actual dependency status does not license. Assessment correctness and governance compliance are therefore separate properties.
...
The actual--assessed--reported--selected separation also suggests that evaluation should localize failure rather than reduce performance to answer accuracy. Assessment accuracy asks whether \(\widehat{\delta}=\delta^{*}\). Reporting fidelity asks whether the communicated epistemic status is no stronger than the assessed status. Governance compliance asks whether \(a_{\mathrm{selected}}\in A_{\Gamma}(\widehat{\delta})\). End-to-end structural compliance asks whether the selected operation is licensed by actual status, \(a_{\mathrm{selected}}\in A_{\Gamma}(\delta^{*})\). These measurements distinguish a system that faithfully acts on a mistaken structural assessment from one that correctly assesses insufficiency and nevertheless violates the corresponding response constraint.
