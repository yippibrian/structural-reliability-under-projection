...
\begin{abstract}
AI systems increasingly make recommendations, explanations, security judgments, and agentic decisions from compressed representations: prompts, summaries, retrieved snippets, logs, metrics, traces, policy artifacts, and interface-bounded context. These representations often omit or collapse distinctions that remain operationally relevant. The resulting failure is one of invariant preservation: the dependency structure required by a downstream claim no longer survives projection in a form that supports the requested answer. When a claim depends on a distinction that the representation does not preserve or make boundedly recoverable, the failure is not merely uncertainty or correctable inference error. It is task-relative representational loss: a failure of the information basis needed to distinguish states that require different answers. For actions and continuations, an additional failure occurs when the represented state does not satisfy, and cannot be returned by bounded correction to, the operational validity conditions of the declared regime.

This paper identifies a structural failure mode in AI reasoning systems: non-identifiability under projection is misclassified as recoverable inference error. Additional reasoning over the same representation can then make an answer more fluent, coherent, or persuasive without restoring the missing distinction. This produces familiar safety and security failures: unsupported root-cause attribution, hallucinated source support, unsafe action under missing state, and metric-driven proxy success related to Goodhart-style proxy collapse~\cite{Goodhart1975}.

The paper distinguishes error, whose required continuation conditions are boundedly restorable within the declared regime, from representational loss, in which the operative representation fails to preserve the distinctions required by a query. It introduces a task-relative exact-sufficiency baseline, distinguishes target-selection loss from downstream representational loss, and classifies surviving dependencies by both structural role and support status. The resulting dependency-status ladder---preserved, reconstructible, traceable, and opaque---governs which answer forms remain licensed after projection. The paper further distinguishes actual, assessed, and reported dependency status, exposing a second-order limitation: successful execution of an admissibility procedure cannot certify that its own application was licensed.

The goal is not to eliminate uncertainty or require abstention. It is to prevent systems from presenting conclusions as determined by representations that do not support them, or actions as permitted when their operational validity has not been established.
\end{abstract}
...
The resulting account is not a machine for producing final answers from fixed premises. It is a framework for keeping the relationship among representations, dependencies, claims, actions, and discovered constraints visible enough that later reasoning can continue without silently strengthening what the available structure supports.
...
David T. Swanson.
\newblock Carriers and Adequacy for Purpose.
\newblock Preprint, 2026.
